 ...
 *
 */
class AdmAclRoleController extends GD_Controller_Admin
{
...
//		$this->view->paginator=$paginator;
	...
	public function addAction(){
		...
		// Manage Form Display
		$form = new ADMIN_Form_Admin_ACL_Role_Add();
		if ($this->_request->isPost()) {
			$formData = $this->_request->getPost();
			if ($form->isValid($formData)) {
				echo 'success';
...
//		$this->view->headLink()->appendStylesheet( $this->baseURL . '/web/css/smoothness/jquery-ui-1.7.2.custom.css');
		...
		$form = new ADMIN_Form_Admin_ACL_Role_Edit($form_options, $form_data);
		if ($this->_request->isPost()) {
			$formData = $this->_request->getPost();
			if ($form->isValid($formData)) {
				echo 'success';
